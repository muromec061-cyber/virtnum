...
function useToken(u) {
    ...
    if (u.tokensUsed >= PLANS[0].tokensPerDay)
        return false;
    u.tokensUsed++;
    return true;
}
...
async function aiChatGitHub(prompt, systemPrompt, modelId) {
    ...
    const resp = await httpsPost('https://models.github.ai/inference/chat/completions', { 'Authorization': `Bearer ${GH_TOKEN}`, 'Content-Type': 'application/json' }, body);
    const d = JSON.parse(resp);
    if (d.error)
        throw new Error(d.error.message || 'GitHub API error');
    if (!d.choices || !d.choices[0])
        throw new Error('No response from GitHub Models');
    return cleanAIResponse(d.choices[0].message.content);
}
async function aiChatLLM7(prompt, systemPrompt, model) {
    const body = JSON.stringify({
        model,
        messages: [
            { role: 'system', content: systemPrompt },
    ...
    const resp = await httpsPost('https://api.llm7.io/v1/chat/completions', { 'Content-Type': 'application/json' }, body);
    const d = JSON.parse(resp);
    if (d.error)
        throw new Error(d.error.message || 'LLM7 error');
    if (!d.choices || !d.choices[0])
        throw new Error('No response from LLM7');
    return cleanAIResponse(d.choices[0].message.content);
}
async function aiChatPollinations(prompt, systemPrompt) {
    const body = JSON.stringify({
        messages: [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: prompt },
    ...
    const resp = await httpsPost('https://text.pollinations.ai/', { 'Content-Type': 'application/json' }, body);
...
async function aiChatWithFallback(prompt, systemPrompt, model) {
    try {
        return await aiChat(prompt, systemPrompt, model);
    }
    catch (e1) {
        console.log(`Model ${model.id} failed: ${e1.message}, falling back...`);
...
bot.start(async (ctx) => {
    ...
    if (adminPremium)
        ...
        text += `🎯 Осталось: ${tokens === -1 ? '∞' : tokens + '/35'} запросов\n\n`;
...
bot.command('models', async (ctx) => {
    ...
    const buttons = MODELS.map((m, i) => {
        ...
        return [telegraf_1.Markup.button.callback(`${active}${m.emoji} ${m.name}`, `m${i}`)];
    });
...
bot.command('profile', async (ctx) => {
    ...
    if (adminPremium)
        ...
        text += `🎯 Запросов сегодня: ${tokens === -1 ? '∞ Безлимит' : tokens + '/35'}\n`;
    await ctx.reply(text, { parse_mode: 'HTML' });
});
// ─── /premium ───
...
bot.command('status', async (ctx) => {
    ...
    if (adminPremium)
        ...
        text += `🎯 Запросов: ${tokens === -1 ? '∞' : tokens + '/35'}\n`;
    await ctx.reply(text, { parse_mode: 'HTML' });
});
// ─── /img ───
...
bot.command('translate', async (ctx) => {
    ...
    if (!text) {
        await ctx.reply('🌐 Напиши: /translate Hello world');
        return;
    }
    try {
...
bot.command('code', async (ctx) => {
    ...
    if (!task) {
        await ctx.reply('💻 Напиши: /code функция сортировки на Python');
        return;
    }
    try {
...
bot.command('weather', async (ctx) => {
    ...
    if (!city) {
        await ctx.reply('🌦 Напиши: /weather Москва');
        return;
    }
    try {
...
bot.command('calc', async (ctx) => {
    ...
    if (!expr) {
        await ctx.reply('🔢 Напиши: /calc 2+2*3');
        return;
    }
    try {
        ...
        await ctx.reply('❌ Неверное выражение');
    }
});
// ─── /pass ───
...
bot.command('ban', async (ctx) => {
    ...
    if (!username) {
        await ctx.reply('Напиши: /ban @username');
        return;
    }
    for (const [, usr] of users) {
...
bot.command('unban', async (ctx) => {
    ...
    const username = ctx.message.text.split(' ')[1]?.replace('@', '');
    if (!username) {
        await ctx.reply('Напиши: /unban @username');
        return;
    }
    for (const [, usr] of users) {
        if (usr.username.toLowerCase() === username.toLowerCase()) {
...
bot.command('premium', async (ctx) => {
    ...
    if (!username) {
        await ctx.reply('Напиши: /premium @username 30');
        return;
    }
    for (const [, usr] of users) {
        if (usr.username.toLowerCase() === username.toLowerCase()) {
...
bot.command('broadcast', async (ctx) => {
    ...
    if (!text) {
        await ctx.reply('Напиши: /broadcast текст сообщения');
        return;
    }
    let sent = 0;
