...
bot.command('ban', async (ctx) => {
  ...
  if (target.username === OWNER_USERNAME) return ctx.reply('⛔ Нельзя заблокировать владельца');
...
bot.command('grant', async (ctx) => {
  const u = getUser(ctx.from!.id, ctx.from!.username, ctx.from!.first_name);
  if (!u.isAdmin) return ctx.reply('⛔ Нет доступа');
  if (u.username !== OWNER_USERNAME) return ctx.reply('⛔ Только владелец может выдавать админку');
