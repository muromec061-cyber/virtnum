...
interface Model {
  idx: number;
  id: string;        // GitHub Models API id or llm7:<model> or sim:<name>
  name: string;
  emoji: string;
  desc: string;
  vendor: string;
  provider: 'github' | 'llm7' | 'sim' | 'pollinations';
  simModel?: string;
  simPrompt?: string;
  llm7Model?: string;
}

const SIM_PROMPTS: Record<string, string> = {
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(options, (res: any) => {
      ...
      res.on('end', () => {
        ...
        if (!parsed) { resolve('⚠️ Нечитаемый ответ AI.'); return; }
        if (parsed.error) { resolve(`⚠️ ${parsed.error.message || 'Ошибка AI'}`); return; }
        if (parsed.choices && parsed.choices[0]) { resolve(cleanAIResponse(parsed.choices[0].message.content)); return; }
...
function aiChatLLM7(prompt: string, systemPrompt: string, modelId: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    const req = https.request(options, (res: any) => {
      ...
      res.on('end', () => {
        const parsed = safeParse(body);
        if (!parsed) { resolve('⚠️ LLM7: нечитаемый ответ.'); return; }
        if (parsed.error) { resolve(`⚠️ LLM7: ${parsed.error.message || parsed.detail || 'ошибка'}`); return; }
        if (parsed.choices && parsed.choices[0]) { resolve(cleanAIResponse(parsed.choices[0].message.content)); return; }
...
async function aiChat(prompt: string, systemPrompt: string, model: Model): Promise<string> {
  ...
  if (model.provider === 'llm7' && model.llm7Model) {
    ...
    if (GITHUB_TOKEN) {
      ...
      if (gh && !gh.startsWith('⚠️')) return gh;
    }
  ...
  if (model.provider === 'pollinations') {
    ...
    if (result && !result.startsWith('⚠️')) return result;
    if (GITHUB_TOKEN) return aiChatGH(prompt, systemPrompt, 'openai/gpt-4o-mini');
    return result;
  }
  ...
  if (model.provider === 'sim' && model.simModel && model.simPrompt) {
    ...
    if (llm7 && !llm7.startsWith('⚠️')) return llm7;
...
let openclawIdx = 0;
function getNextOpenClawModel(): string {
  const model = OPENCLAW_MODELS[openclawIdx % OPENCLAW_MODELS.length];
  openclawIdx++;
  return model;
}

async function openclawChat(prompt: string, systemPrompt: string): Promise<string> {
  ...
  // Fallback to LLM7
  const llm7 = await aiChatLLM7(prompt, systemPrompt, 'qwen3-235b');
  if (!llm7.startsWith('⚠️')) return llm7;
  return aiChatPoll(prompt, systemPrompt);
}
...
function ghAPI(path: string, method: string = 'GET', body?: any): Promise<any> {
  return new Promise((resolve) => {
    ...
    const req = https.request(options, (res: any) => {
      ...
      res.on('end', () => { try { resolve(JSON.parse(respBody)); } catch { resolve(null); } });
    });
...
bot.start(async (ctx) => {
  ...
  await ctx.replyWithHTML(text, Markup.inlineKeyboard([
    ...
    [Markup.button.callback('🦅 OpenClaw', 'openclaw_menu'), Markup.button.callback('📦 GitHub', 'github_menu')],
    [Markup.button.callback('🎨 Генератор артов', 'art_gen'), Markup.button.callback('📋 Помощь', 'help_cmd')],
    [Markup.button.callback('📊 Статус', 'status_cmd')],
  ]));
});
...
bot.command('ai', async (ctx) => {
  ...
  if (!prompt) {
    await ctx.replyWithHTML(`<b>🧠 AI готов!</b>\n\n${div}\n\nНапиши вопрос:\n<code>/ai объясни квантовую запутанность</code>`, Markup.inlineKeyboard([[Markup.button.callback('🤖 Выбор модели', 'model_menu')]]));
    return;
  }
  const session = getSession(ctx.from!.id);
...
bot.command('model', async (ctx) => {
  ...
  for (const v of VENDORS) {
    ...
    if (count > 0) text += `  ${v} (${count})\n`;
  }
  text += `\n<i>Выбери провайдера 👇</i>`;
  const rows: any[] = [];
  for (const v of VENDORS) {
    const count = MODELS.filter(m => m.vendor === v).length;
    if (count > 0) rows.push([Markup.button.callback(`${v} (${count})`, `v_${v.replace(/[^a-zA-Z]/g, '')}`)]);
  }
...
VENDORS.forEach(v => {
  ...
  bot.action(cbId, async (ctx) => {
    ...
    for (const m of models) text += `${m.emoji} <b>${m.name}</b> — ${m.desc}\n`;
    text += `\n<i>Выбери 👇</i>`;
    const btns = models.map(m => Markup.button.callback(`${m.emoji} ${m.name}`, `m${m.idx}`));
    const rows: any[] = [];
    for (let i = 0; i < btns.length; i += 2) rows.push(btns.slice(i, i + 2));
    rows.push([Markup.button.callback('⬅️ К провайдерам', 'model_menu'), Markup.button.callback('🏠 Меню', 'back_start')]);
...
MODELS.forEach(m => {
  bot.action(`m${m.idx}`, async (ctx) => {
    ...
    await ctx.editMessageText(
      `✅ <b>${m.emoji} ${m.name}</b> выбрана!\n\n${div}\n${m.desc}\nПровайдер: ${m.vendor}\n\n<i>/ai твой вопрос</i>`,
      { parse_mode: 'HTML', ...Markup.inlineKeyboard([
        [Markup.button.callback('⬅️ К провайдерам', 'model_menu')],
...
bot.action('ai_chat', async (ctx) => {
  ...
  await ctx.editMessageText(`<b>🧠 AI готов!</b>\n\n${div}\n\nНапиши свой вопрос текстом 👇`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('⬅️ Меню', 'back_start')]]) });
});
...
bot.on('text', async (ctx) => {
  ...
  if (session.awaitingInput === 'ai') {
    ...
    try {
      ...
      await ctx.telegram.deleteMessage(ctx.chat.id, waitMsg.message_id).catch(() => {});
      await ctx.replyWithHTML(esc(result.substring(0, 4000)), Markup.inlineKeyboard([
        [Markup.button.callback('🧠 Ещё', 'ai_chat')],
        [Markup.button.callback('⬅️ Меню', 'back_start')],
      ]));
    } catch (e: any) {
      await ctx.telegram.deleteMessage(ctx.chat.id, waitMsg.message_id).catch(() => {});
    ...
    try {
      ...
      await ctx.telegram.deleteMessage(ctx.chat.id, waitMsg.message_id).catch(() => {});
      await ctx.replyWithPhoto({ url }, { caption: `🎨 <i>${esc(prompt.substring(0, 100))}</i>`, parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🎨 Ещё', 'art_gen'), Markup.button.callback('⬅️ Меню', 'back_start')]]) });
    } catch (e: any) {
      await ctx.telegram.deleteMessage(ctx.chat.id, waitMsg.message_id).catch(() => {});
...
bot.command('team', async (ctx) => {
  ...
  for (const a of TEAM_AGENTS) text += `${a.name} — /${a.cmd} (${a.desc})\n`;
  await ctx.replyWithHTML(text, Markup.inlineKeyboard([
    [Markup.button.callback('🏠 Меню', 'back_start')],
  ]));
});
...
bot.command('super', async (ctx) => {
  ...
  for (const a of SUPER_AGENTS) text += `${a.name} — /${a.cmd} (${a.desc})\n`;
  await ctx.replyWithHTML(text, Markup.inlineKeyboard([
    [Markup.button.callback('🏠 Меню', 'back_start')],
  ]));
});
...
// ─── AI-powered commands ───
async function aiCommand(ctx: any, systemPrompt: string, userPrompt: string) {
  const session = getSession(ctx.from!.id);
  const model = getModel(session.modelIdx);
...
bot.command('shorten', (ctx) => {
  ...
  if (!url) { ctx.reply('Использование: /shorten <URL>'); return; }
...
bot.command('img', async (ctx) => {
  ...
  if (!prompt) { ctx.replyWithHTML('🎨 Использование: <code>/img космический кот</code>'); return; }
...
bot.action('art_gen', async (ctx) => {
  ...
  await ctx.editMessageText(`<b>🎨 Генератор артов</b>\n\n${div}\n\nОпиши что нарисовать 👇`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('⬅️ Меню', 'back_start')]]) });
});
...
bot.command('openclaw', async (ctx) => {
  ...
  if (!prompt) {
    await ctx.replyWithHTML(`<b>🦅 OpenClaw</b>\n\n${div}\n\nАвтономный AI-агент для сложных задач.\n\nИспользование:\n<code>/openclaw напиши план запуска стартапа</code>`,
      Markup.inlineKeyboard([[Markup.button.callback('🦅 Запустить', 'openclaw_start'), Markup.button.callback('⬅️ Меню', 'back_start')]]));
    return;
  }
  const waitMsg = await ctx.replyWithHTML('🦅 <i>OpenClaw работает...</i>');
  try {
...
bot.action('openclaw_menu', async (ctx) => {
  await ctx.answerCbQuery();
  await ctx.editMessageText(`<b>🦅 OpenClaw</b>\n\n${div}\n\nАвтономный AI-агент с ротацией ${OPENCLAW_MODELS.length} моделей.\n\nИспользование:\n<code>/openclaw &lt;задача&gt;</code>`,
    { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🦅 Запустить', 'openclaw_start'), Markup.button.callback('⬅️ Меню', 'back_start')]]) });
});
...
bot.action('openclaw_start', async (ctx) => {
  ...
  await ctx.editMessageText(`<b>🦅 OpenClaw готов!</b>\n\n${div}\n\nОпиши задачу 👇`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('⬅️ Меню', 'back_start')]]) });
});
...
bot.command('gh_repos', async (ctx) => {
  ...
  if (!data || !Array.isArray(data)) { ctx.reply('⚠️ Ошибка GitHub API'); return; }
...
bot.command('gh_profile', async (ctx) => {
  ...
  if (!data) { ctx.reply('⚠️ Ошибка'); return; }
...
bot.command('gh_commits', async (ctx) => {
  ...
  if (!data || !Array.isArray(data)) { ctx.reply('⚠️ Репо не найдено'); return; }
...
bot.command('gh_search', async (ctx) => {
  ...
  if (!q) { ctx.reply('Использование: /gh_search <query>'); return; }
  const data = await ghAPI(`/search/repositories?q=${encodeURIComponent(q)}&per_page=5`);
  if (!data || !data.items) { ctx.reply('⚠️ Ошибка поиска'); return; }
...
bot.action('status_cmd', async (ctx) => {
  ...
  await ctx.editMessageText(text, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🏠 Меню', 'back_start')]]) });
});
